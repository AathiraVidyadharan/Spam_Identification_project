...
Alice trains a spam classifier with naive bayes on some data she
...
def preprocess_data():
    ...
    X = check_array(X, accept_sparse='csr', ensure_2d=True)  # Convert to dense matrix
...
class Alice:
    """
    Trains a Naive Bayes model on plaintext data,
...
# make fitting and predictions on dataset.
   ...
   #secure the parameters with encryption. 
    def encrypt_model(self):
        # Get the model parameters
        feature_log_prob_ = self.model.feature_log_prob_
        class_log_prior_ = self.model.class_log_prior_

        # Encrypt the model parameters
        encrypted_feature_log_prob_ = [
            [self.pubkey.encrypt(val) for val in tqdm(row)]
            for row in tqdm(feature_log_prob_)
        ]
        encrypted_class_log_prior_ = [self.pubkey.encrypt(val) for val in class_log_prior_]

        return encrypted_feature_log_prob_, encrypted_class_log_prior_
...
class Bob:
    ...
    #setting parameters received from Alice on his data.
    def set_model(self, feature_log_prob_, class_log_prior_):
...
#computes encrypted score on his data.
    def encrypted_score(self, x):
        scores = []
        for class_index in range(len(self.class_log_prior_)):
            score = self.class_log_prior_[class_index]
            _,idx = x.nonzero()
            for i in idx:
                score += x[0, i] * self.feature_log_prob_[class_index][i]
        return score
...
#Alice demonstrating accuracy of model without knowing Bob data.   
  ...
  #Alice encrypts the models parameters.  
    print("Alice: Encrypting classifier")
    with timer() as t:
        encrypted_feature_log_prob_, encrypted_class_log_prior_ = alice.encrypt_model()

   #compute encrypted score on his data and evaluates.
    print("Bob: Scoring with encrypted classifier")
    bob = Bob(alice.pubkey)
    bob.set_model(encrypted_feature_log_prob_, encrypted_class_log_prior_)
    with timer() as t:
